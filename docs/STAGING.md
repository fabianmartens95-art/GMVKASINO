...
3. requires an HTTPS `STAGING_BASE_URL`,
4. rejects an identical `PRIMARY_BASE_URL` when that secret is configured,
5. runs the existing remote `npm run smoke` against staging,
6. records the tested commit SHA and result in the GitHub Actions step summary.
