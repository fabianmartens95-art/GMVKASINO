...
export async function verifyStagingRevision({
  baseUrl,
  expectedRevision,
  fetchImpl = fetch,
} = {}) {
  ...
  if (!response.ok) {
    throw new Error(`Staging liveness returned HTTP ${response.status}`)
  }

  const payload = await response.json()
  if (payload?.ok !== true || payload?.status !== 'live' || payload?.mode !== 'demo') {
    throw new Error('Staging liveness response is not the expected GMVKASINO demo contract')
  }

  const deployed = normalizedSha(payload.revision, 'deployed revision')
  if (deployed !== expected) {
    throw new Error(`Staging revision mismatch: expected ${expected}, deployed ${deployed}`)
  }

  return { ok: true, revision: deployed }
}
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch((error) => {
    console.error(JSON.stringify({
      ok: false,
      error: error?.message || 'Staging revision verification failed',
    }))
    process.exitCode = 1
  })
}
