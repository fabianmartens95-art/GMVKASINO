...
- Double-entry ledger transactions and immutable ledger entries
...
- Session invalidation does not delete account or ledger history
...
Every ledger transaction must balance to zero. A demo wager of `5.00` and win of `12.00`, for example, produces postings between the user's DEMO wallet and the DEMO house ledger account; the wallet's cached atomic balance is updated inside the same PostgreSQL transaction.
...
Complete M6 with authenticated accounts and stronger ledger reconciliation/observability, then add backup/restore procedures and a staging deployment. Real-money and cryptocurrency movement remain out of scope.
